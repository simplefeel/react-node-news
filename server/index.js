    ...
    .end(function (err, sres) {
        ...
        $('.day_part .w_tit a').each(function (idx, element) {
            items.push({
                title:$(this).text(),
                href: $(this).attr('href'),
                time: $(this).parent('.w_tit').siblings('.info').find('.source').text()
            })
        });
...
app.get('/news/:pageSize', function (req, res, next) {
    superagent
    .get(`http://www.tmtpost.com/nictation/${req.params.pageSize}`)
    .end(function (err, sres) {
        if (err) {
            return next(err);
        }
